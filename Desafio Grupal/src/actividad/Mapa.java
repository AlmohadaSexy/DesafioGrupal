...
public class Mapa {
	...
	/**
	 * Añade una conexion de reparto "Origen -> Destino" que deberá cumplirse en el problema
	...
	public void encontrarCamino(String inicio) {
		if(existsNode(inicio)) {
			...
			for(int ii = 0; ii < recorridos.size(); ii++) {
				System.out.println("Recorrido: " + recorridos.get(ii) + ", Distancia: " + distanciaRecorridos.get(ii) + " km.");
			}
			System.out.println();
			System.out.println("Camino mas eficaz");
			System.out.println("Recorrido: " + recorridos.get(calcularCaminoMasEficaz()) + ", Distancia: " + distanciaRecorridos.get(calcularCaminoMasEficaz()) + " km.");
			//hacer
        } else {
